...
## Phase-State Validation Gate (M3.5)
- Phase transition decisions must be generated by machine-evaluable rules in `config/geo_phase_policy.json`.
- Required reason codes must be present for every hold/promotion result.
- Any missing reason-code emission is a validation failure.
- Promotion to Phase B is blocked unless all thresholds in `spec/PHASE_STATE_POLICY.md` pass.

## GEO Escalation Automation
- Weekly geo readiness escalation is evaluated automatically.
